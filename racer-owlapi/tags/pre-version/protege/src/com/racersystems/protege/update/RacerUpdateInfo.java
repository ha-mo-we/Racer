...
public class RacerUpdateInfo {
    ...
    public RacerUpdateInfo(Version version, URL downloadURL) {
     ...
     * {@link com.racersystems.protege.RacerInstalledBinaryInfo#getOSIdentifier()})
     ...
     *                   not available.
     */
    public static RacerUpdateInfo read(URL updateFileLocation) throws Exception {
        ...
        if (downloadString != null) {
            ...
            if (index > -1) {
                downloadString = downloadString.substring(0, index) + OSID_PREFIX +
                        RacerInstalledBinaryInfo.getInstance().getOSIdentifier() + ".zip";
            } else {
                downloadString += OSID_PREFIX + RacerInstalledBinaryInfo.getInstance().getOSIdentifier() + ".zip";
            }
            downloadURL = new URL(downloadString);
        }
        RacerUpdateInfo info = new RacerUpdateInfo(version, downloadURL);
