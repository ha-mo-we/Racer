...
public class UpdateInstaller {
    ...
    /**
     * The file available at {@link RacerProUpdateInfo#getDownloadURL()} will be downloaded to a temporary directory. If it
     ...
     * @see com.racersystems.protege.RacerProInstalledBinaryInfo#getBinaryInstallationDirectory()
     */
    public File downloadFile(RacerProUpdateInfo info) throws IOException {
        ...
        if (downloadURL.getFile().endsWith(".zip")) {
            extract(tmpFile, RacerProInstalledBinaryInfo.getInstance().getBinaryInstallationDirectory());
        } else {
            copy(tmpFile, RacerProInstalledBinaryInfo.getInstance().getBinaryInstallationDirectory());
        }
        //Create updateFile in installation directory:
        File file = new File(RacerProInstalledBinaryInfo.getInstance().getBinaryInstallationDirectory(), RacerProUpdateInfo.UPDATE_INFO_NAME);
        if (!file.exists())
            file.createNewFile();
        PrintWriter writer = new PrintWriter(new FileOutputStream(file));
        writer.println(RacerProUpdateInfo.VERSION_PROP_KEY + "=" + info.getAvailableVersion().toString());
        writer.flush();
        writer.close();
