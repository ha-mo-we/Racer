...
public class RacerInterfaceTest extends AbstractReasonerOntologyCase {
    ...
    public void testEquivalentClasses() throws Exception {
        ...
        Node<OWLClass> n = reasoner.getEquivalentClasses(getOWLClass("A"));
        assertTrue(n.contains(getOWLClass("A")));
    }

    public void testEquivalentProperties() throws Exception {
        Set<OWLAxiom> axioms = new HashSet<OWLAxiom>();
        axioms.add(getDataFactory().getOWLEquivalentObjectPropertiesAxiom(getOWLObjectProperty("propA"), getOWLObjectProperty("propB")));
        manager.addAxioms(getRootOntology(), axioms);

        Node<OWLObjectPropertyExpression> node = reasoner.getEquivalentObjectProperties(getOWLObjectProperty("propA"));
        assertTrue(node.contains(getOWLObjectProperty("propB")));

        node = reasoner.getEquivalentObjectProperties(getOWLObjectProperty("Q"));
        assertTrue(node.getSize() == 1); //it must contain Q
        assertTrue(node.contains(getOWLObjectProperty("Q")));
    ...
    /*  public static void main(String[] args) throws Exception{
        Socket s = new Socket("134.60.75.180", 8088);
        s.connect(new InetSocketAddress("134.60.75.258", 8088));
    }
    */
